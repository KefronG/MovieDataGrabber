...
if __name__ == '__main__':
    ...
    for i in range(10):
        ...
        print("Genres:")
        for g in range(len(genres)): movie_genres.append(genres[g]['name'])
        for g in range(len(genres)):print(movie_genres[g])
        ...
        #starring cast
        print("Starring: ")
        crew = getcredits(imdb_ids[i])[0]['cast']
        crew_scores = [c for c in crew if crew[len(crew) - 1]['popularity']]
        pop_score = []
        for c in range(len(crew_scores)):pop_score.append(crew_scores[c]['popularity'])
        pop_score.sort()
        ...
        #Directors
        Directors =[]
        lst_directors = [credit for credit in getcredits(imdb_ids[i])[0]['crew'] if credit["job"] == "Director"]
        for people in range(len(lst_directors)): Directors.append(lst_directors[people]['name'])
        for name in range(len(lst_directors)):print(f"\nDirector: {Directors[name]}")
